	...
	for (var i = 0; i < boid_list.length; i++) {
		...
		if (
			Math.sqrt(Math.pow(b1.x - b2.x, 2) + Math.pow(b1.y - b2.y, 2)) <
				boidViewingDist &&
			Math.abs(
				normalizeRad(-Math.atan2(b2.y - b1.y, b2.x - b1.x) - b1.angle)
			) < boidViewingAngle
		) {
			neighbors.push(b2);
		}
...
function separation(boid, neighbors) {
	...
	neighbors.forEach(elem => {
		...
		var dist = Math.sqrt(
			Math.pow(elem.x - boid.x, 2) + Math.pow(elem.y - boid.y, 2)
		);
...
function alignment(boid, neighbors) {
	...
	var weight =
		Math.abs(diffAngle) > 0.2 * Math.PI
			? 1
			: Math.abs(diffAngle) / (0.2 * Math.PI);
...
function drawStats(col, sep, ali, coh) {
	...
	for (var i = 0; i < result.length; i++) {
		...
		ctx.fillRect(
			canvas.width - barWidth * (result.length - i),
	...
	ctx.fillText(
		"Col",
		canvas.width - barWidth * 3.7,
		canvas.height - barHeightMax - 10
	);
	ctx.fillText(
		"Sep",
		canvas.width - barWidth * 2.7,
		canvas.height - barHeightMax - 10
	);
	ctx.fillText(
		"Ali",
		canvas.width - barWidth * 1.7,
		canvas.height - barHeightMax - 10
	);
	ctx.fillText(
		"Coh",
		canvas.width - barWidth * 0.7,
		canvas.height - barHeightMax - 10
	);
	ctx.fillText(
		Math.round(sum * 100) / 100,
		canvas.width - barWidth * 4.5,
		canvas.height - barHeightMax / 2
	);

	// Draw UI border
	ctx.beginPath();
	ctx.rect(
		canvas.width - barWidth * result.length,
		canvas.height - barHeightMax,
		barWidth * result.length,
		barHeightMax
	);
	ctx.stroke();
